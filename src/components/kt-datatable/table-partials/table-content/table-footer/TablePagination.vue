      ...
      <ul class="pagination">
        <li class="paginate_button page-item">
          <input
            ...
            min="1"
            :max="totalPages"
            @keydown.enter="onClickGoToPage"
            class="form-control page-input p-1"
          />
        </li>
        <li class="paginate_button page-item">
          <button class="page-link" @click="onClickGoToPage">Перейти</button>
        </li>
        <li
  ...
  setup(props, { emit }) {
    ...
    const onClickGoToPage = () => {
      if (
        ...
        goToPageInput.value <= props.totalPages
      ) {
        emit("page-change", goToPageInput.value);
      }
    };
